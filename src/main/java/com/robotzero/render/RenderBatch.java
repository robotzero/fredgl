...
import com.robotzero.infrastructure.DataType;
...
public class RenderBatch implements Comparable<RenderBatch> {
    ...
    public void loadVertexProperties(int index) {
        ...
        for (int i=0; i < 4; i++) {
            ...
            vertices[offset] = sprite.getGameObject().getTransform().position.x + (xAdd * sprite.getGameObject().getTransform().scale.x);
            if (sprite.getGameObject().getTransform().scale.x < 0) vertices[offset] += sprite.getGameObject().getTransform().scale.x * -1;
            vertices[offset + 1] = sprite.getGameObject().getTransform().position.y + (yAdd * sprite.getGameObject().getTransform().scale.y);
            if (sprite.getGameObject().getTransform().scale.y < 0) vertices[offset + 1] += sprite.getGameObject().getTransform().scale.y * -1;
            vertices[offset + 2] = sprite.getGameObject().getzIndex();
